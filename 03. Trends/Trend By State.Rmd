...
# Average Confirmed Cases per state
nationwide_avg = df_daily_case_by_state_region %>% summarise(nationwide_avg = round(mean(Confirmed_Cases_cum),0)) %>% pull(nationwide_avg)
nationwide_avg
nationwide_med = df_daily_case_by_state_region %>% summarise(nationwide_med = round(median(Confirmed_Cases_cum),0)) %>% pull(nationwide_med)
nationwide_med
```

```{r}
max_df <- df_daily_case_by_state_region %>% 
  group_by(States) %>% 
